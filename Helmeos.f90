!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This files contains subroutine which related to the Helmholtz EOS !
! Including subroutine for finding pressure and their deriatives    !
! and inverting between pressure, energy and temperature	    !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! The following part are provided from timmers et. al !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! open the file (use softlinks to input the desired table)
       ...
       open(unit=999,file='./Library/helm_table.dat',status='old')
...
! read the helmholtz free energy and its derivatives
       do j=1,jmax
        ...
        do i=1,imax
         dsav = dlo + (i-1)*dstp
         d(i) = 10.0d0**(dsav)
         read(999,*) f(i,j),fd(i,j),ft(i,j),fdd(i,j),ftt(i,j),fdt(i,j), &
...
! read the pressure derivative with density table
       do j=1,jmax
        do i=1,imax
         read(999,*) dpdf(i,j),dpdfd(i,j),dpdft(i,j),dpdfdt(i,j)
        enddo
       enddo
...
! read the electron chemical potential table
       do j=1,jmax
        do i=1,imax
         read(999,*) ef(i,j),efd(i,j),eft(i,j),efdt(i,j)
        enddo
       enddo
...
! read the number density table
       do j=1,jmax
        do i=1,imax
         read(999,*) xf(i,j),xfd(i,j),xft(i,j),xfdt(i,j)
        enddo
       enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine invert_helm_pd(rho_input, p_input, abar_input, zbar_input, ye_input, temp_input, temp_out)
	USE DEFINITION, ONLY: density, pressure, temperature, epsilon
      include 'implno.dek'
      include 'const.dek'
      include 'vector_eos.dek'
...
double precision                     :: rho_input, p_input, abar_input, zbar_input, ye_input, temp_input, temp_out
...
jlo_eos = 1
jhi_eos = 1
den_row(1)        = (rho_input/density)
ptot_row(1) 	  = (p_input/pressure)
temp_row(1)       = (temp_input/temperature)    
...
subroutine invert_helm_ed(epsilon_input, rho_input, abar_input, zbar_input, ye_input, temp_input, temp_output, epsilon_output, p_out)
	USE DEFINITION, ONLY: density, pressure, temperature, epsilon
      include 'implno.dek'
      include 'const.dek'
      include 'vector_eos.dek'
...
double precision                     :: epsilon_input, epsilon_output, rho_input, abar_input, &
                                        zbar_input, ye_input, temp_input, temp_output, p_out
...
jlo_eos = 1
jhi_eos = 1
etot_row(1)       = (epsilon_input/epsilon)
temp_row(1)       = (temp_input/temperature)
den_row(1)        = (rho_input/density)         
abar_row(1)       = abar_input
zbar_row(1)       = MIN(zbar_input, ye_input*abar_input)
...
! call eos one more time with the converged value of the density
      ...
      temp_output = temp_row(1)*temperature
      epsilon_output = etot_row(1)*epsilon
      p_out = ptot_row(1)*pressure
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Here are the subroutine that is computed by ours !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This subroutine convert a given (p,T,A,Z) to rho !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
SUBROUTINE HELMEOS_PtOR(p_in, temp_in, abar_in, zbar_in, ye_in, rho_trial, rho_out)
USE DEFINITION
IMPLICIT NONE
INCLUDE 'vector_eos.dek'

! REAL PARAMETER !
REAL (DP), INTENT(IN) :: p_in, temp_in, abar_in, zbar_in, ye_in, rho_trial
REAL (DP), INTENT(OUT) :: rho_out

! ASSIGN INPUT !
ptot_Row(1) = (p_in/pressure)
temp_row(1) = (temp_in/temperature)
abar_row(1) = abar_in
zbar_row(1) = MIN(zbar_in, ye_in*abar_in)
den_row(1) = (rho_trial/density)
...
! ASSIGN OUTPUT !
rho_out = (den_row(1)*density)

END SUBROUTINE HELMEOS_PTOR

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This subroutine convert a given (rho,T,A,Z) to P and related derivatives !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
SUBROUTINE HELMEOS_RtoP(rho_in, temp_in, abar_in, zbar_in, ye_in, p_out, dpdrho_out, dpdepsilon_out, eta_out)
USE DEFINITION, ONLY: DP, density, pressure, temperature, epsilon
IMPLICIT NONE             
INCLUDE 'vector_eos.dek'
INCLUDE 'const.dek'

! Real parameter !
REAL (DP), INTENT(IN) :: rho_in, temp_in, abar_in, zbar_in, ye_in             
REAL (DP), INTENT(OUT) :: p_out, dpdrho_out, dpdepsilon_out, eta_out

! assign input !
den_Row(1) = (rho_in/density)
temp_row(1) = (temp_in/temperature)
abar_row(1) = abar_in
zbar_row(1) = MIN(zbar_in, ye_in*abar_in)
...
! assign output !
p_out = (ptot_row(1)*pressure)
dpdrho_out = (dpd_row(1)*epsilon)
dpdepsilon_out = (dpT_row(1) / deT_row(1)*density)
eta_out = etaele_row(1) + (me*clight**2)/(kerg*temp_row(1))

END SUBROUTINE HELMEOS_RToP 

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This subroutine convert a given (rho,T,A,Z) to P and related derivatives !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
SUBROUTINE HELMEOS_RtoE(rho_in, temp_in, abar_in, zbar_in, ye_in, epsilon_out, depsilondT_out)
USE DEFINITION
IMPLICIT NONE             
INCLUDE 'vector_eos.dek'
...
! Real parameter !
REAL (DP), INTENT(IN) :: rho_in, temp_in, abar_in, zbar_in, ye_in
REAL (DP), INTENT(OUT) :: epsilon_out, depsilondT_out
...
! ! assign output !
epsilon_out = (etot_row(1)*epsilon)
depsilondT_out = (deT_row(1)*epsilon/temperature)

END SUBROUTINE HELMEOS_RToE

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This subroutine converts (rho,eps,A,Z) to T !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! Real parameters !
REAL (DP) :: temp_old, temp_new
REAL (DP) :: epsilon_old, epsilon_new
...
DO j = 1, length_step_part_2
	IF(rho2(j) > rho2_a) THEN
		...
		! Back up !
		epsilon_old = epsilon2(j)
		temp_old = temp2(j)

		! Invert the temperature !
		CALL invert_helm_ed(epsilon_old, rho2(j), abar2(j), zbar2(j), ye2(j), temp_old, temp_new, epsilon_new, dummy)
		temp2(j) = temp_new

		! Do that for SNeIa !
		IF(fusion_flag == 1) THEN
			epsilon2(j) = epsilon_new
			IF(temp2(j) < temp2_min) THEN
				temp2(j) = temp2_min
				CALL HELMEOS_RTOE(rho2(j), temp2(j), abar2(j), zbar2(j), ye2(j), epsilon_new, dummy)
				epsilon2(j) = epsilon_new
			ELSEIF(temp2(j) > temp2_max) THEN
				temp2(j) = temp2_max
				CALL HELMEOS_RTOE(rho2(j), temp2(j), abar2(j), zbar2(j), ye2(j), epsilon_new, dummy)                             
				epsilon2(j) = epsilon_new
			ENDIF
...
REAL (DP), INTENT(IN) :: temp_in, rho_in, abar_in, zbar_in, ye_in

! assign input, the mutted one are old schemes !
den_row(1) = (rho_in/density)
temp_row(1) = (temp_in/temperature)
abar_row(1) = abar_in
zbar_row(1) = MIN(zbar_in, ye_in * abar_in)
...
! assign output, choose in between different sound speed calculation !
cs_out = (cs_row(1)/clight)

END SUBROUTINE HELMEOS_CS