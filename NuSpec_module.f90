...
do j = 1, length_step_part_2
	if(rho2(j) > rho2_flame_min .and. temp2(j) > 1.0D0) then

		! Do the standard bilinear interpolation 
		irho = INT((LOG10(rho2 (j)/density) - 7.0D0) / 0.1D0) + 1
		itemp = INT(temp2 (j)) 
		drho = ((LOG10(rho2 (j)/density) - 7.0D0) / 0.1D0 + 1.0D0) - DBLE(irho)
		dtemp = temp2 (j) - DBLE(itemp) 
		...
		do i = 1, 10
			...
			! 2006 Pair Neutrino !
			(4.5215D25 * emiss * 0.1425776426D0 / (0.08614D0 * temp2 (j)) * &
			(nu_freq / 0.08614D0 / temp2 (j)) ** 3.180657028D0 * &
			EXP(-1.108192299D0 * nu_freq / 0.08614D0 / temp2 (j)) + &

			! 2007 Plasma Neutrino !
			2.115D30 * (0.08614D0 * temp2 (j)) * (mass_t * 0.511D0 ** 2) ** 3 * &
			EXP(-nu_freq / 0.08614D0 / temp2 (j))) & 
		
			! Sum all volume contribution !
			* vol2 (j)/length**3
...
! open the files !
open(unit=898, file='./Library/nu_mass.dat', action='read')
open(unit=899, file='./Library/nu_emiss.dat', action='read')
...
! output the data !
open(unit=601, file='./Outfile/Star_WENO_NuSpec_0.dat', action='write', position='append')
