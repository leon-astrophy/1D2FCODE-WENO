...
USE DEFINITION, ONLY : DP, epsilon
IMPLICIT NONE
...
! Open file in the low-Ye patch !
open(unit=441, file='./Library/ecaptable_Ye040_wNabi_full.dat', action='read')
...
do i = 0, ye_row
	do j = 0, temp_row
		do k = 0, rho_row      
			...
			eneutable(i,j,k) = input5*epsilon * 4.9282D-6
...
USE DEFINITION, ONLY : DP, density
IMPLICIT NONE
...
! Input quantities !
real (DP), INTENT(IN) :: rho_in, temp_in, ye_in
 
! Output quantities
real (DP), INTENT(OUT) :: ecaprate_out, eneurate_out
